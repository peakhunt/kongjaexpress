...
__watcher_start(watcher_t* watcher, uint8_t target)
{
  ...
  {
    ev_io_start(task_self()->ev_loop, __get_ev_io(watcher, target));
  }
}
...
__watcher_stop(watcher_t* watcher, uint8_t target)
{
  ...
  if(watcher->flags & WATCHER_STARTED)
  {
    ev_io_stop(task_self()->ev_loop, __get_ev_io(watcher, target));
  }
}
...
watcher_start(watcher_t* watcher)
{
  ...
  {
    ev_io_start(task_self()->ev_loop, &watcher->rx_io);
  }

  if(watcher->flags & WATCHER_TX_WATCH)
  {
    ev_io_start(task_self()->ev_loop, &watcher->tx_io);
  }

  if(watcher->flags & WATCHER_ER_WATCH)
  {
    ev_io_start(task_self()->ev_loop, &watcher->er_io);
  }
}
...
watcher_stop(watcher_t* watcher)
{
  ...
  if(watcher->flags & WATCHER_RX_WATCH)
  {
    ev_io_stop(task_self()->ev_loop, &watcher->rx_io);
  }

  if(watcher->flags & WATCHER_TX_WATCH)
  {
    ev_io_stop(task_self()->ev_loop, &watcher->tx_io);
  }

  if(watcher->flags & WATCHER_ER_WATCH)
  {
    ev_io_stop(task_self()->ev_loop, &watcher->er_io);
  }
}
